...
export const applyTransformsToPoint = (
	...
	// Важен порядок применения: scale и rotate обычно применяются до translate
	// Твой `TransformGroup` уже задает определенный порядок, но здесь мы его
	// должны воспроизвести явно. Идеально - сначала scale, потом rotate, потом translate.
	// Давай пройдемся по массиву transforms и применим их.

	// 1. Применяем все трансформации к координатам точки
	// Эта простая итерация не учитывает порядок (scale/rotate до translate)
	// Для этого нужна более сложная матричная математика или разделение трансформаций.
	// Давай пока реализуем простой, но рабочий вариант, а потом улучшим.

	// --- УЛУЧШЕННЫЙ ПОДХОД с правильным порядком ---
	let totalScaleX = 1,
		totalScaleY = 1,
		totalAngle = 0,
		totalTranslateX = 0,
